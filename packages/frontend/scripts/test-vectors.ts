...
async function main() {
	if (process.argv.includes("--help") || process.argv.includes("-h")) {
		...
		console.log(
			"Environment: BORROWER_ADDRESS, CREDIT_POLICY_ADDRESS, SCORE_REGISTRY_ADDRESS, RPC_URL, CHAIN_ID, NONCE",
		);
		return;
	}
	...
	const creditPolicyAddress =
		process.env.CREDIT_POLICY_ADDRESS ??
		process.env.VITE_CREDIT_POLICY_ADDRESS ??
		deployment.creditPolicyAddress;
	const scoreRegistryAddress =
		process.env.SCORE_REGISTRY_ADDRESS ??
		process.env.VITE_SCORE_REGISTRY_ADDRESS ??
	...
	const nonce = process.env.NONCE
		? Number(process.env.NONCE)
		: Math.floor(Date.now() / 1000) >>> 0;

	if (!creditPolicyAddress) {
		throw new Error(
			"Missing CREDIT_POLICY_ADDRESS or VITE_CREDIT_POLICY_ADDRESS",
		);
	}
