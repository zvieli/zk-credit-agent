...
import {
	generateLoanProof,
	requestAxiomRoot,
} from "../../backend/src/axiom_service";
...
const combinedProofFixture = JSON.parse(
	readFileSync(
		new URL("../../contracts/test/data/combined_proof.hex", import.meta.url),
		"utf8",
	),
) as CombinedProofFixture;

const TARGET_USER_ADDRESS = getAddress(
	"0xE71CbF47Fff309813bcea54f3ecF49a5F129264D",
);
const defaultNonce = combinedProofFixture.scoreInputs.metadata.nonce;
const proofBlockNumber = BigInt(
	combinedProofFixture.scoreInputs.metadata.blockNumber,
);
...
async function waitForVerifiedRoot(params: {
	...
	while (Date.now() - startedAt < timeoutMs) {
		const stateRoot = await readVerifiedRootOnChain(
			params.rpcUrl,
		...
		if (
			!isZeroBytes32(stateRoot) &&
			stateRoot.toLowerCase() === params.expectedStateRoot.toLowerCase()
		) {
			return stateRoot;
		}
			...
			try {
				if (message.type === "ensure-engine") {
					this.emit({
						id: message.id,
						type: "ready",
						circuitName: message.circuitName,
					});
					return;
				}
				...
				if (message.type === "compute-public-commitment") {
					...
					const syncApi = await (
						bbModule as unknown as {
							BarretenbergSync: {
								initSingleton: () => Promise<{
									pedersenHash: (params: {
										inputs: Uint8Array[];
										hashIndex: number;
					...
					this.emit({
						id: message.id,
						type: "commitment",
						commitment: bytesToHex(response.hash),
					});
					return;
				}
...
function installNodeShims() {
	...
	globalScope.window =
		globalScope.window ??
		(globalScope as unknown as typeof globalThis & { fetch?: typeof fetch });
	globalScope.fetch = ((input: RequestInfo | URL, init?: RequestInit) => {
		...
		if (
			requestUrl &&
			(requestUrl.port === "3001" || requestUrl.port === "") &&
...
beforeAll(() => {
	...
	localProofRpcServer = createServer(async (request, response) => {
		try {
			...
			sendJson(response, 500, {
				error: error instanceof Error ? error.message : "RPC server error",
			});
		}
	});
	...
	localBackendServer = createServer(async (request, response) => {
		...
		try {
			if (
				url.pathname === "/api/request-axiom-root" &&
				request.method === "POST"
			) {
				const body = (await readJsonBody(request)) as
					| {
			...
			if (
				url.pathname === "/api/generate-loan-proof" &&
				request.method === "POST"
			) {
				const body = (await readJsonBody(request)) as
					| {
				...
				const proofInputs = await buildScoreProofInputs({
					userAddress: body.userAddress,
					contractAddress:
						body.scoreRegistryAddress ??
						readDeploymentConfig().scoreRegistryAddress ??
						"0x65a44ee2218a4d56fbf6a7d1a65d267b65347e0b",
					nonce: body.nonce,
				...
				sendJson(response, 200, {
					...
					creditPolicyAddress:
						...
						readDeploymentConfig().creditPolicyAddress ??
...
describe("frontend logic integration", () => {
	it("syncs MPT data, waits for Axiom verification, and validates backend proof output", async () => {
		...
		const scoreRegistryAddress = resolveOrFallbackAddress(
			deployment.scoreRegistryAddress,
			"0x65a44ee2218a4d56fbf6a7d1a65d267b65347e0b",
		);
		const creditPolicyAddress = resolveOrFallbackAddress(
		...
		expect(normalizeHex(syncInputs.metadata.userAddress)).toBe(
			TARGET_USER_ADDRESS.toLowerCase(),
		);
		expect(normalizeHex(syncInputs.metadata.contractAddress)).toBe(
			scoreRegistryAddress.toLowerCase(),
		);
		...
		expect(syncInputs.public_commitment.toLowerCase()).toBe(
			syncInputs.metadata.publicCommitment.toLowerCase(),
		);
		...
		expect(requestResult.blockNumber).toBe(
			syncInputs.metadata.blockNumber.toString(),
		);
		...
		const relayer = await startAxiomRelayer({
			...
			extraData: encodeAbiParameters(parseAbiParameters("uint256"), [
				syncInputs.metadata.blockNumber,
			]),
			signerPrivateKey: (process.env.AXIOM_QUERY_PRIVATE_KEY ??
				process.env.AGENT_PRIVATE_KEY) as Hex,
			startBlock:
				syncInputs.metadata.blockNumber > 1n
					? syncInputs.metadata.blockNumber - 1n
					: 0n,
		});
		...
		try {
			...
			expect(verifiedRoot.toLowerCase()).toBe(
				syncInputs.metadata.stateRoot.toLowerCase(),
			);

			const proofResult = await postBackendJson<GenerateLoanProofResult>(
				"/api/generate-loan-proof",
				{
					userAddress: TARGET_USER_ADDRESS,
					blockNumber: syncInputs.metadata.blockNumber,
					nonce: defaultNonce,
			...
			expect(proofResult.publicInputs[0]?.toLowerCase()).toBe(
				syncInputs.metadata.publicCommitment.toLowerCase(),
			);
			expect(proofResult.stateRoot.toLowerCase()).toBe(
				syncInputs.metadata.stateRoot.toLowerCase(),
			);
			expect(proofResult.blockNumber).toBe(
				syncInputs.metadata.blockNumber.toString(),
			);
			expect(proofResult.creditPolicyAddress.toLowerCase()).toBe(
				requestResult.creditPolicyAddress.toLowerCase(),
			);
		} finally {
			relayer.stop();
			await relayer.done;
