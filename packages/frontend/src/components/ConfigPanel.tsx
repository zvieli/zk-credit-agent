...
export default function ConfigPanel(props: any) {
	...
	return (
		<GlassCard
			className={`utility-panel utility-panel--sidebar ${isOpen ? "is-open" : "is-closed"}`}
		>
				...
				<>
					...
					<div className="field-grid">
						...
						<label>
							...
							<input
								value={creditPolicyAddress}
								onChange={(event) => setCreditPolicyAddress(event.target.value)}
								placeholder="0x..."
							/>
						</label>
						<label>
							...
							<input
								value={scoreRegistryAddress}
								onChange={(event) =>
									setScoreRegistryAddress(event.target.value)
								}
								placeholder="0x..."
							/>
						</label>
