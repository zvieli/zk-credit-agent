...
export default function ActionHub(props: {
  ...
  const { flowPhases, flowPhase, status, scoreInputs, axiomDispatch, combinedProof, proofHash, scoreTxHash, handleRunAxiomNoirFlow, handleAxiomSync, userAddress, setUserAddress } = props;
  const activeIndex = Math.max(0, flowPhases.findIndex((item) => item.phase === flowPhase));
  const activeStep = flowPhases[activeIndex] ?? flowPhases[0];
  const activeStatus =
    flowPhase === 'IDLE' ? status.sync.status :
    flowPhase === 'AXIOM_REQUESTED' ? status.request.status :
    flowPhase === 'AXIOM_VERIFIED' ? status.verify.status :
    flowPhase === 'NOIR_PROVING' ? status.proof.status :
    status.submit.status;
  const activeMessage =
    flowPhase === 'IDLE' ? status.sync.message :
    flowPhase === 'AXIOM_REQUESTED' ? status.request.message :
    flowPhase === 'AXIOM_VERIFIED' ? status.verify.message :
    flowPhase === 'NOIR_PROVING' ? status.proof.message :
    status.submit.message;

  const canSync = Boolean(userAddress) && status.sync.status !== 'working';
  const canRun = status.sync.status === 'complete' && Boolean(scoreInputs) && status.request.status !== 'working' && status.verify.status !== 'working' && status.proof.status !== 'working' && status.submit.status !== 'working';
        ...
        <label className="hub-address-field">
          ...
          <div className="hub-address-control">
            <input
              ...
              spellCheck={false}
            />
            <button type="button" className="hub-paste-button" onClick={handlePaste} aria-label="Paste target wallet address">
        ...
        <StepCard
          ...
          details={axiomDispatch ? (
            <div className="metrics metrics--compact">
              ...
              <div>
                <span>Tx</span>
                <strong>{axiomDispatch.txHash.slice(0, 10)}…{axiomDispatch.txHash.slice(-8)}</strong>
              </div>
            </div>
          ) : null}
        />
          ...
          details={combinedProof ? (
            <div className="metrics metrics--compact">
              ...
              <div>
                ...
                <strong>{proofHash ? `${proofHash.slice(0, 10)}…${proofHash.slice(-8)}` : 'pending'}</strong>
              </div>
            </div>
          ) : null}
        />
          ...
          details={scoreTxHash ? (
            <div className="metrics metrics--compact">
              <div>
                ...
                <strong>{scoreTxHash.slice(0, 10)}…{scoreTxHash.slice(-8)}</strong>
              </div>
            </div>
          ) : null}
        />
