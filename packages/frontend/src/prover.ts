...
const DEFAULT_USDC_ADDRESS = getAddress(
	import.meta.env.VITE_USDC_ADDRESS ??
		"0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48",
);
const DEFAULT_USDT_ADDRESS = getAddress(
	import.meta.env.VITE_USDT_ADDRESS ??
		"0xdAC17F958D2ee523a2206206994597C13D831ec7",
);
const DEFAULT_AAVE_POOL_ADDRESS = getAddress(
...
function postProofWorkerMessage<T>(
	...
	return new Promise<T>((resolve, reject) => {
		const timeoutId = window.setTimeout(() => {
			...
			reject(
				new Error(
					`SES_DETECTION_ERROR: proof worker did not respond within ${timeoutMs}ms`,
				),
			);
		}, timeoutMs);
...
function resolveRpcUrl(explicitRpcUrl?: string) {
	return (
		explicitRpcUrl ?? import.meta.env.VITE_RPC_URL ?? "http://127.0.0.1:8545"
	);
}
...
function flattenStorageProof(storageProof: BackendProofData["storageProof"]) {
	...
	if (typeof firstEntry === "string") {
		if (storageProof.length === 1 && firstEntry.toLowerCase() === "0x80") {
			throw new Error(
				"Data not found in this block. Ensure you are on the correct fork.",
			);
		}
	...
	if (
		...
		Array.isArray(firstEntry.proof)
	) {
...
async function ensureEngine(circuitName: ProofCircuitName): Promise<void> {
	...
	await postProofWorkerMessage<void>(
		{ id: requestId, type: "ensure-engine", circuitName },
		900000,
	);
}

export async function warmProofEngine(
...
function packStaticPathNodes(nodes: readonly Uint8Array[]): number[][] {
	if (nodes.length > STATIC_PATH_NODE_LIMIT) {
		throw new Error(
			`Static path proof exceeds ${STATIC_PATH_NODE_LIMIT} nodes`,
		);
	}
...
function packStaticPathScalars(values: readonly number[]) {
	if (values.length > STATIC_PATH_NODE_LIMIT) {
		throw new Error(
			`Static path proof exceeds ${STATIC_PATH_NODE_LIMIT} steps`,
		);
	}
...
function listItemAt(
	data: Uint8Array,
	listPayloadOffset: number,
	targetIndex: number,
): [number, DecodedRlpItem] {
	let offset = listPayloadOffset;
	let item: DecodedRlpItem = {
		kind: 0,
		payloadOffset: listPayloadOffset,
		payloadLen: 0,
		totalLen: 0,
	};
...
function describeStaticPathProof(
	nodesHex: readonly string[],
	rootHash: Hex,
	key: Uint8Array,
): {
	ordered: Uint8Array[];
	...
	try {
		...
		let keyOffset = 0;
		const rootIndex = remaining.findIndex(
			(node) => keccak256(node) === rootHash,
		);
		...
		for (let step = 0; step < STATIC_PATH_NODE_LIMIT; step++) {
			...
			const [firstItemOffset, firstItem] = listItemAt(
				currentNode,
			...
			if (isCompactNode) {
				...
				const { isLeaf, consumed } = verifyCompactPath(
					currentNode,
				...
				const compactNibbles = compactPathToNibbles(
					currentNode,
					firstItem.payloadOffset,
					firstItem.payloadLen,
				);
				...
				if (nextIndex < 0) {
					if (reference.length > 0 && reference.length < 32) {
						console.info("[sync] inline extension child fallback", {
					...
					throw new Error(
						"Unable to resolve extension child in static path proof",
					);
				}
			...
			if (keyOffset === 64) {
				const [terminalItemOffset, terminalItem] = listItemAt(
					currentNode,
					decodedNode.payloadOffset,
					16,
				);
			...
			const [candidateOffset, candidateChild] = listItemAt(
				currentNode,
				decodedNode.payloadOffset,
				branchIndex,
			);
			...
			const reference = currentNode.slice(
				candidateChild.payloadOffset,
				candidateChild.payloadOffset + candidateChild.payloadLen,
			);
			const nextIndex = remaining.findIndex((candidate) =>
				nodeMatchesReference(candidate, reference),
			);
			...
			if (nextIndex < 0) {
				if (reference.length > 0 && reference.length < 32) {
					console.info("[sync] inline branch child fallback", {
						referenceLen: reference.length,
					});
		...
		console.info("[sync] keyless static path fallback", {
...
function extractAccountLeafFieldHints(leafValue: Uint8Array) {
	...
	if (storageRootItem.payloadLen !== 32) {
		throw new Error(
			`Unexpected account leaf storage root length: ${storageRootItem.payloadLen}`,
		);
	}

	return {
		balanceOffset:
			balanceItem.payloadOffset +
			(accountRecordPayload === leafValue
				? 0
				: listItemAt(leafValue, leafNode.payloadOffset, 1)[0]),
		...
		storageRoot: accountRecordPayload.slice(
			storageRootItem.payloadOffset,
...
async function getUserFeaturesAndSignature(
	...
	logger?.fetching?.(
		`Fetching proof data for ${validatedAddress} at ${validatedContract}...`,
	);
	...
	if (!response.ok) {
		throw new Error(
			rawResponse ||
				`Request to /api/get-proof-data failed with ${response.status}`,
		);
	}
...
export async function buildLoanProofInputs(
	...
	const inferredStateRoot = resolveTrieRoot(
		stateRoot as Hex | undefined,
		recoveredStateRoot,
	);
	...
	const accountTrieKeyBytes = hexToBytes(
		keccak256(hexToBytes(normalizedAavePoolAddress)),
	);
	...
	const accountPath = describeStaticPathProof(
		accountProof,
		inferredStateRoot,
		accountTrieKeyBytes,
	);
	...
	const accountNodeLens = packStaticPathScalars(accountPath.nodeLens).map(
		(value) => Number(value),
	);
	const accountNodeTypes = packStaticPathScalars(accountPath.nodeTypes).map(
		(value) => Number(value),
	);
	const accountPathOffsets = packStaticPathScalars(accountPath.pathOffsets).map(
		(value) => Number(value),
	);
	const accountPathLens = packStaticPathScalars(accountPath.pathLens).map(
		(value) => Number(value),
	);
	const accountValueOffsets = packStaticPathScalars(
		accountPath.childOffsets,
	).map((value) => Number(value));
	const accountValueLens = packStaticPathScalars(accountPath.childLens).map(
		(value) => Number(value),
	);
	const accountBranchIndices = packStaticPathScalars(
		accountPath.branchIndices,
	).map((value) => Number(value));
	...
	try {
		...
		console.info("[sync] account leaf storage root fallback", {
			reason: error instanceof Error ? error.message : String(error),
		});
	}
	...
	const resolvedStorageRoot = resolveTrieRoot(
		accountLeafStorageRoot,
		recoveredStorageRoot ?? (storageHash as Hex),
	);
	...
	const storagePath = describeStaticPathProof(
		storageProof,
		resolvedStorageRoot,
		storageKeyBytes,
	);
	...
	const storageNodeLens = packStaticPathScalars(storagePath.nodeLens).map(
		(value) => Number(value),
	);
	const storageNodeTypes = packStaticPathScalars(storagePath.nodeTypes).map(
		(value) => Number(value),
	);
	const storagePathOffsets = packStaticPathScalars(storagePath.pathOffsets).map(
		(value) => Number(value),
	);
	const storagePathLens = packStaticPathScalars(storagePath.pathLens).map(
		(value) => Number(value),
	);
	const storageValueOffsets = packStaticPathScalars(
		storagePath.childOffsets,
	).map((value) => Number(value));
	const storageValueLens = packStaticPathScalars(storagePath.childLens).map(
		(value) => Number(value),
	);
	const storageBranchIndices = packStaticPathScalars(
		storagePath.branchIndices,
	).map((value) => Number(value));
	const storageValueOffset = Number(
		storagePath.childOffsets[storagePath.childOffsets.length - 1] ?? 0,
	);
	const storageValueLen = Number(
		storagePath.childLens[storagePath.childLens.length - 1] ?? 0,
	);
	const storageRealSteps = Number(storagePath.ordered.length);

	const storageLeaf =
		storagePath.ordered[storagePath.ordered.length - 1] ?? new Uint8Array();
	...
	if (storageValueLen !== 0) {
		...
		for (let index = 0; index < 32; index++) {
			if (index >= start) {
				storageWord[index] =
					storageLeaf[storageValueOffset + index - start] ?? 0;
			}
		}
	}
	...
	if (
		publicCommitment.toLowerCase() !== expectedPublicCommitment.toLowerCase()
	) {
...
export async function generateProof(
	...
	await ensureEngine(circuitName);
	return postProofWorkerMessage<GeneratedProof>(
		{ id: requestId, type: "generate-proof", circuitName, inputs },
		900000,
	);
}
