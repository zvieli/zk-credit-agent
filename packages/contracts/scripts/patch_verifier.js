...
const vkMatch = content.match(
	/Honk\.VerificationKey memory vk = Honk\.VerificationKey\(\{([\s\S]+?)\}\);/,
);
if (vkMatch) {
	const vkBlock = vkMatch[1];
	const getAssign = (key) => {
		const regex = new RegExp(
			`${key}:\\s+Honk\\.G1Point\\(\\{([\\s\\S]+?)\\}\\)`,
			"m",
		);
	...
	if (libStartMatch) {
		...
		const libEnd =
			content.indexOf("}", content.indexOf("}", vkFuncEnd) + 1) + 1;

		const prefix = content.substring(0, libStart);
		const suffix = content.substring(libEnd);

		content =
			prefix + `library HonkVerificationKey {${newKeyLoader}\n}` + suffix;
	}
}
