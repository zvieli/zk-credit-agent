...
async function main() {
	...
	if (rpcUrl.includes("127.0.0.1") || rpcUrl.includes("localhost")) {
		...
		console.log(
			`Funded deployment account for local testing: ${account.address} with 0.1 ETH`,
		);
	}
	...
	const combinedVerifierArtifact = JSON.parse(
		readFileSync(
			join(CONTRACTS_OUT, "combined_verifier.sol/HonkVerifier.json"),
			"utf-8",
		),
	);
	...
	const combinedHash = await walletClient.deployContract({
		...
		bytecode: linkLibrary(
			combinedVerifierArtifact.bytecode.object,
			libAddress,
		) as Hex,
	});
	const combinedVerifierAddr = (
		await publicClient.waitForTransactionReceipt({ hash: combinedHash })
	).contractAddress!;

	console.log("Deploying ScoreRegistry...");
	const registryArtifact = JSON.parse(
		readFileSync(
			join(CONTRACTS_OUT, "ScoreRegistry.sol/ScoreRegistry.json"),
			"utf-8",
		),
	);
	...
	const relayerAddr = (
		await publicClient.waitForTransactionReceipt({ hash: relayerHash })
	).contractAddress!;
