...
export {
	buildAndSendAxiomStateRootQuery,
...
export const AAVE_V3_POOL_ADDRESS = getAddress(
...
function resolveRemoteRpcUrls() {
	const candidates = [
		process.env.PROFILE_RPC_URL,
...
export async function getUserFeaturesAndSignature(
	...
	console.log("[rpc] getUserAccountData:start", {
	...
	console.log("[rpc] getUserAccountData:done", {
		userAddress: validatedAddress,
	});

	console.log("[rpc] getUserConfiguration:start", {
		userAddress: validatedAddress,
		contractAddress: validatedContract,
	});
	...
	console.log("[rpc] getUserConfiguration:done", {
		userAddress: validatedAddress,
	});
	...
	const toUint32 = (num: number) =>
		Math.max(0, Math.min(Math.floor(num), 0xffffffff));
	const maxDiversity = 100;

	const repayment_consistency_score = Math.min(
		Number(ethBalance / 10n ** 17n) * 10,
		500,
	);
	...
	for (let retry = 0; retry <= maxRetries; retry++) {
		...
		if (isLocal) {
			console.log(
				`[rpc] rawBlock keys for block ${currentBlock.number}:`,
				rawBlock ? Object.keys(rawBlock) : "null",
			);
			if (rawBlock) {
				console.log(
					`[rpc] rawBlock stateRoot: ${rawBlock.stateRoot || rawBlock.state_root || "MISSING"}`,
				);
			}
		}
		...
		let resolvedStateRoot =
			overrides?.stateRoot ??
			resolveBlockStateRoot(rawBlock) ??
			resolveBlockStateRoot(currentBlock);

		// Fallback to source RPC if Anvil returns empty state root
		if (isLocal && isEmptyStateRoot(resolvedStateRoot)) {
			console.log(
				`[rpc] Local Anvil returned empty stateRoot for block ${currentBlock.number}. Attempting to fetch from source RPC...`,
			);
			for (const sourceRpcUrl of resolveRemoteRpcUrls()) {
				try {
					const sourceClient = createPublicClient({
						chain: mainnet,
						transport: http(sourceRpcUrl, { timeout: 30000 }),
					});
					const sourceBlock = await sourceClient.getBlock({
						blockNumber: currentBlock.number,
					});
					const sourceRawBlock = (await sourceClient.request({
						method: "eth_getBlockByNumber",
						params: [`0x${currentBlock.number.toString(16)}`, false],
					})) as Record<string, unknown> | null;

					const sourceRoot =
						resolveBlockStateRoot(sourceRawBlock) ??
						resolveBlockStateRoot(sourceBlock);
					if (!isEmptyStateRoot(sourceRoot)) {
						resolvedStateRoot = sourceRoot;
						console.log(
							`[rpc] Successfully fetched stateRoot from source RPC for block ${currentBlock.number}: ${resolvedStateRoot}`,
						);
						break;
					}

					console.log(
						`[rpc] Source RPC also returned empty stateRoot for block ${currentBlock.number}: ${sourceRpcUrl}`,
					);
				} catch (error) {
					console.error(
						`[rpc] Failed to fetch block from source RPC ${sourceRpcUrl}:`,
						error,
					);
				}
			}
		...
		if (isLocal && isEmptyStateRoot(resolvedStateRoot)) {
			console.log(
				`[rpc] No source RPC configured or all source RPCs returned empty stateRoot for block ${currentBlock.number}.`,
			);
		}

		console.log("[rpc] getBlock:done", {
			blockNumber: currentBlock.number,
		...
		console.log("[rpc] getBlock:missing-stateRoot-keys", {
			...
			rawBlockKeys:
				rawBlock && typeof rawBlock === "object" ? Object.keys(rawBlock) : [],
		});

		if (retry === maxRetries) {
			throw new Error(
				"Fetched block has an empty state root. The fork might be out of sync.",
			);
		}

		const nextBlockNumber = isLocal
			? currentBlock.number > 1n
				? currentBlock.number - 1n
				: 0n
			: currentBlock.number > 200n
				? currentBlock.number - 200n
				: 0n;

		console.log(
			`[rpc] Block ${currentBlock.number.toString()} has empty stateRoot. ${isLocal ? "Retrying previous block" : "Jumping back 200 blocks"}...`,
		);
	...
	const stablecoin_ratio =
		colBase > 0
			? Math.min(100, Math.floor((stablecoin_balance / colBase) * 100))
			: 0;
	...
	const historical_bonus = Math.min(
		Math.floor(Number(ethBalance / 10n ** 17n)) * 10,
		500,
	);
	...
	const predictedScore =
		userConfig === 0n
			...
			: Math.max(
					0,
					Math.min(
						Math.floor(netEquity / 20 + hfBonus + walletWealth - penalty),
						1000,
					),
				);
	...
	const storageAddress = overrides?.storageProofAddress
		? getAddress(overrides.storageProofAddress)
		: AAVE_V3_POOL_ADDRESS;
	const storageSlot =
		overrides?.storageProofSlot ??
		getAaveUserConfigStorageSlot(validatedAddress);
	console.log("[rpc] getProof:start", {
		userAddress: validatedAddress,
		storageAddress,
		storageSlot,
		blockNumber: blockNumber.toString(),
	...
	const storageProofValue =
		proofAny.storageProof?.[0]?.value ?? `0x${userConfig.toString(16)}`;
