...
async function main() {
    ...
    let allLogs: any[] = [];
    const BLOCKS_TO_SEARCH = 250n;
    const CHUNK_SIZE = 10n;

    for (let currentBlock = latestBlock - BLOCKS_TO_SEARCH; currentBlock <= latestBlock; currentBlock += CHUNK_SIZE) {
        const toBlock = (currentBlock + CHUNK_SIZE - 1n) > latestBlock ? latestBlock : (currentBlock + CHUNK_SIZE - 1n);
        try {
            ...
            console.log(`Error fetching blocks ${currentBlock} to ${toBlock}: ${e.message}`);
        }
    }
    ...
    for (const user of users) {
        ...
        // Get ETH Balance
        const ethBalanceRaw = await client.getBalance({ address: user as `0x${string}` });
        ...
        let hfBonus = diversity > 3 ? 150 : diversity * 50;
        ...
        if (usdcBalance > 100) {
            ...
            console.log(`🏥 Health Factor: ${healthFactor > 100 ? "Safe" : healthFactor.toFixed(2)} | Liquidations: ${pastLiquidations}`);
            ...
            if (finalPredictedScore >= 300) {
                const rate = finalPredictedScore >= 850 ? "2%" : (finalPredictedScore >= 600 ? "5%" : "8%");
