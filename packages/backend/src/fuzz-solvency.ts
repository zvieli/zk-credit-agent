...
function loadDeploymentDefaults() {
	const raw = readFileSync(
		path.resolve(FRONTEND_PUBLIC, "deployment.json"),
		"utf8",
	);
...
function parseIterations(argv: string[]) {
	const flagIndex = argv.findIndex(
		(value) => value === "--iterations" || value === "-i",
	);
...
async function generateProof(
	...
	const { witness } = await proofEngine.noir.execute(
...
function makeLeafNode(word: Uint8Array, declaredValueLength = word.length) {
	...
	const listHeaderLength =
		nodeBytes.length - pathItem.length - valueItem.length;
	const pathOffset = listHeaderLength + (pathItem.length - pathPayload.length);
	const pathLen = pathPayload.length;
	const valueOffset =
		listHeaderLength +
		pathItem.length +
		(valueItem.length - declaredValueLength);
...
async function buildStorageInputs(
	word: Uint8Array,
	options?: {
		declaredValueLength?: number;
		isSolvent?: boolean;
	...
	const { key, node, pathOffset, pathLen, valueOffset, valueLen } =
		makeLeafNode(word, declaredValueLength);
...
async function expectProofOutcome(
	label: string,
	inputs: Record<string, unknown>,
	shouldSucceed: boolean,
) {
	console.log(
		`[fuzz] ${label} -> ${shouldSucceed ? "expect success" : "expect failure"}`,
	);
	...
	try {
		...
		if (!shouldSucceed) {
			throw new Error(
				`Expected ${label} to fail, but proof generation succeeded with ${proof.publicInputs.length} public inputs.`,
			);
		}
...
async function fuzzReferenceSolvency(iterations: number) {
	console.log(
		`Starting reference solvency fuzzing for ${iterations} iterations...`,
	);
	...
	for (const edgeWord of edgeWords) {
		...
		if (reference.isSolvent !== expected) {
			throw new Error(
				`Edge case reference mismatch for word ${bytesToHex(edgeWord)}.`,
			);
		}

		if (
			bytesToHex(edgeWord) === bytesToHex(new Uint8Array(32)) &&
			expected !== false
		) {
	...
	for (let iteration = 0; iteration < iterations; iteration++) {
		...
		const reference = referenceEvaluateAaveUserConfig(
			wordToBigInt(word),
		).isSolvent;
...
async function runCircuitCases() {
	...
	await expectProofOutcome(
		"empty account",
		await buildStorageInputs(emptyWord, { isSolvent: false }),
		true,
	);
	...
	await expectProofOutcome(
		"debt-only trap",
		await buildStorageInputs(debtOnlyWord, { isSolvent: false }),
		true,
	);
	...
	await expectProofOutcome(
		...
		await buildStorageInputs(collateralOnlyWord, { isSolvent: true }),
		true,
	);

	const randomWord = randomBytes(32);
	const expected = computeSolventFromWord(randomWord);
	const goodInputs = await buildStorageInputs(randomWord, {
		isSolvent: expected,
	});
	await expectProofOutcome("random good proof", goodInputs, true);

	const flippedInputs = await buildStorageInputs(randomWord, {
		isSolvent: !expected,
	});
	await expectProofOutcome("flipped solvency bit", flippedInputs, false);

	const malformedPrefixInputs = await buildStorageInputs(randomWord, {
		mutatePrefix: 0xc0,
		isSolvent: expected,
	});
	await expectProofOutcome("invalid prefix", malformedPrefixInputs, false);

	const fiftyFiveByteInputs = await buildStorageInputs(randomBytes(55), {
		declaredValueLength: 55,
		isSolvent: false,
	});
	await expectProofOutcome("55-byte RLP boundary", fiftyFiveByteInputs, false);

	const fiftySixByteInputs = await buildStorageInputs(randomBytes(56), {
		declaredValueLength: 56,
		isSolvent: false,
	});
	await expectProofOutcome("56-byte RLP boundary", fiftySixByteInputs, false);

	const overlongLengthInputs = await buildStorageInputs(randomWord, {
		declaredValueLength: 500,
		isSolvent: expected,
	});
	...
	const deepPathInputs = {
		...
		nodes: [
			...(goodInputs.nodes as number[][]),
			new Array<number>(STATIC_PATH_NODE_BYTES).fill(0),
		],
	};
...
async function main() {
	...
	if (process.argv.includes("--help") || process.argv.includes("-h")) {
		...
		console.log(
			"Optional env: AGENT_PRIVATE_KEY, PROOF_RPC_URL, RPC_URL, AAVE_V3_POOL_ADDRESS",
		);
		return;
	}
