import { createPublicClient, http, keccak256, encodeAbiParameters, parseAbiItem, parseAbiParameters, getAddress, formatUnits, toHex } from 'viem';
...
function resolveSourceRpcUrl() {
	...
	if (process.env.RPC_URL && !process.env.RPC_URL.includes('127.0.0.1') && !process.env.RPC_URL.includes('localhost')) {
...
const AAVE_POOL_ADDRESS = getAddress(process.env.AAVE_V3_POOL_ADDRESS || '0x87870Bca3F3fD6335C3F4ce8392D69350B4fA4E2');
const USDC_ADDRESS = getAddress(process.env.USDC_ADDRESS || '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48');
...
const FALLBACK_LOOKBACK_WINDOWS = [25000, 50000, 100000, 250000, 500000, 1000000];
const CHUNK_SIZE = BigInt(process.env.SEED_CHUNK_SIZE || '10');
const INJECTED_NONCE = 500n;
const USDC_BALANCE_SLOT = 9n;
const AAVE_POOL_USERS_CONFIG_SLOT = BigInt(process.env.AAVE_POOL_USERS_CONFIG_SLOT || '53');
const ENABLE_AAVE_POOL_CONFIG = (process.env.SEED_AAVE_USER_CONFIG || 'true').toLowerCase() !== 'false';

const supplyEvent = parseAbiItem('event Supply(address indexed reserve, address user, address indexed onBehalfOf, uint256 amount, uint16 indexed referralCode)');
...
const repayEvent = parseAbiItem('event Repay(address indexed reserve, address indexed user, address indexed repayer, uint256 amount, bool useATokens)');
...
const liquidationEvent = parseAbiItem('event LiquidationCall(address indexed collateralAsset, address indexed debtAsset, address indexed user, uint256 debtToCover, uint256 liquidatedCollateralAmount, address liquidator, bool receiveAToken)');
...
function classifyProfile(profile: RealProfile) {
	...
	if (profile.usdcBalance > 1_000_000_000n || profile.ethBalance > 5n * 10n ** 18n) {
		return 'Whale';
	}
...
async function discoverRealProfiles(client = createClient(resolveSourceRpcUrl())) {
	...
	for (const windowBlocks of windows) {
		const fromBlock = latestBlock > BigInt(windowBlocks) ? latestBlock - BigInt(windowBlocks) : 0n;
			...
			for (const { event, userField } of activityEvents) {
				try {
					...
					for (const log of logs) {
						...
						const candidate = userField === 'user' ? logArgs.user : logArgs.onBehalfOf;
						if (!candidate) {
							continue;
						}
						...
						if (!userToBlock.has(address) || userToBlock.get(address)! < blockNumber) {
							userToBlock.set(address, blockNumber);
						}
					}
				} catch (error) {
					const message = error instanceof Error ? error.message : String(error);
					console.log(`Skipping ${event.name} range ${current}..${toBlock}: ${message}`);
				}
			}
	...
	if (userToBlock.size < REAL_PROFILE_LIMIT) {
		console.log(`Found only ${userToBlock.size} candidates after adaptive search; continuing with the available set.`);
	}

	const candidates = [...userToBlock.entries()].sort((left, right) => Number((right[1] ?? 0n) - (left[1] ?? 0n)));
	...
	for (const [user, activityBlock] of candidates) {
		...
		try {
			const [accountData, ethBalance, usdcBalance, realNonce, userConfigResult] = await Promise.all([
				client.readContract({
					address: AAVE_POOL_ADDRESS,
					abi: poolAbi,
			...
			if (usdcBalance === 0n && accountData[0] === 0n && accountData[1] === 0n) {
				continue;
			}
...
async function seedProfile(
	...
	const usdcSlot = keccak256(
		encodeAbiParameters(parseAbiParameters('address, uint256'), [localAddress, USDC_BALANCE_SLOT]),
	);
	await setStorageWord(client, USDC_ADDRESS, usdcSlot as Address, profile.usdcBalance);

	if (seedPoolConfig) {
		const poolConfigSlot = keccak256(
			encodeAbiParameters(parseAbiParameters('address, uint256'), [localAddress, AAVE_POOL_USERS_CONFIG_SLOT]),
		);
		await setStorageWord(client, AAVE_POOL_ADDRESS, poolConfigSlot as Address, profile.userConfig);
	}
...
function printSummary(records: SeedRecord[]) {
	...
	console.table(
		records.map((record) => ({
			...
			proofs: record.accountProofOk && record.usdcProofOk && (!ENABLE_AAVE_POOL_CONFIG || record.poolConfigProofOk) ? 'ok' : 'partial',
		})),
	);
}
...
async function main() {
	...
	const localAccounts = await localClient.request({ method: 'eth_accounts' } as any) as string[];
	const seededAccounts = localAccounts.slice(1, 1 + profiles.length).map((value) => getAddress(value));

	if (seededAccounts.length < profiles.length) {
		throw new Error(`Anvil returned only ${seededAccounts.length} unlocked accounts; need ${profiles.length} to seed accounts 1-${profiles.length}.`);
	}
	...
	for (let index = 0; index < profiles.length; index += 1) {
		...
		const { usdcSlot, seedPoolConfig } = await seedProfile(localClient, anvilAddress, profile);
		await verifyProofPaths(localClient, anvilAddress, usdcSlot as Address, seedPoolConfig);
		...
		console.log(`Seeded Anvil account #${index + 1} with real user ${profile.address}`);
	}
